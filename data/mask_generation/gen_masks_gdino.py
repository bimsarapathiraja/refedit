...
def main():
    ...
    for id in tqdm(keys):
        ...
        try:
            edit_objs = [info["edit_1"]['editing_object'], info["edit_2"]['editing_object']]
            descriptive_expressions = [info["edit_1"]['descriptive_referring_expression'], info["edit_2"]['descriptive_referring_expression']]
        except:
        ...
        for edit_obj, ref_exp in zip(edit_objs, descriptive_expressions):
            ...
            annotated_frame = annotate(image_source=image_source, boxes=final_box, logits=max_conf, phrases=[edit_obj])
            annotated_frame = annotated_frame[...,::-1]
            Image.fromarray(annotated_frame).save(f"{img_save_dir_pth}/{det_img_name}")

            # crop and save the image
            cropped_image = crop_image(image_source, final_box)
            Image.fromarray(cropped_image).save(f"{img_save_dir_pth}/{cropped_img_name}")

            segmented_frame_masks = segment(image_source, sam_predictor, boxes=final_box, device=device)
            annotated_frame_with_mask = draw_mask(segmented_frame_masks[0][0], annotated_frame)
            Image.fromarray(annotated_frame_with_mask).save(f"{img_save_dir_pth}/{seg_img_name}")

            mask = segmented_frame_masks[0][0].cpu().numpy()
            Image.fromarray(mask).save(f"{img_save_dir_pth}/{mask_img_name}")
            # print("\n\n")
        # break
