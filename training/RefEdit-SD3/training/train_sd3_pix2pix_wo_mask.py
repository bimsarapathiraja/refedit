...
def main():
    ...
    # ********************************************************************************************************************
    # Since we are finetuning the trained SD3 model, we don't need to initialize the new channel of DIT.
    # ********************************************************************************************************************
    ...
    print("column_names", column_names) # changed
    ...
    print("original_image_column", original_image_column)
    print("edit_prompt_column", edit_prompt_column)
    print("edited_image_column", edited_image_column)
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    if (
                        ...
                        if ckpt_save_bool:
                            ckpt_save_pth = os.path.join(args.output_dir, f"checkpoint-{global_step}") # change
                            pipeline.save_pretrained(ckpt_save_pth) # change
                            logger.info(f"Saved state to {ckpt_save_pth}") # change
    ...
    if accelerator.is_main_process:
        ...
        pipeline.save_pretrained(f"{args.output_dir}/final")
