...
class EditDatasetTrain(Dataset):
    def __init__(
        self,
        path: str,
        split: str = "train",
        ...
        self.seeds_file = {
            "train": "TIGER-Lab/OmniEdit-Filtered-1.2M",
        }[split]

        omnidataset = load_dataset(self.seeds_file)
        self.seeds = omnidataset['train']
    ...
    def __getitem__(self, i: int) -> dict[str, Any]:
        ...
        prompt = random.choice(seed["edited_prompt_list"])

        image_0 = seed["src_img"].convert("RGB")
        image_1 = seed["edited_img"].convert("RGB")
...
class EditDatasetVal(Dataset):
    def __init__(
        ...
        self.seeds_file = {
            "val": "/home/cr8dl-user/DATA_bim/kingkong2/updated_mb_stage1/val/edit_sessions_mb.json",
        }[split]
    ...
    def __getitem__(self, i: int) -> dict[str, Any]:
        ...
        seed = self.seeds[self.seed_keys[i]]

        prompt = seed['instruction']

        image_0 = Image.open(seed['input']).convert("RGB")
        image_1 = Image.open(seed['output']).convert("RGB")
...
class EditDatasetEval(Dataset):
    def __init__(
        ...
        print("*******************************************")
        print("ERROR: EditDatasetEval is not implemented")
        print("*******************************************")
